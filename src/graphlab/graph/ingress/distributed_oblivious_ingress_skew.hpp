...
#ifndef GRAPHLAB_DISTRIBUTED_OBLIVIOUS_INGRESS_SKEW_HPP
#define GRAPHLAB_DISTRIBUTED_OBLIVIOUS_INGRESS_SKEW_HPP
...
namespace graphlab {
   ...
   */
  template<typename VertexData, typename EdgeData>
  class distributed_oblivious_ingress_skew: 
  ...
  public:
    distributed_oblivious_ingress_skew(distributed_control& dc, graph_type& graph, bool usehash = false, bool userecent = false) :
      base_type(dc, graph),
    ...
    ~distributed_oblivious_ingress_skew() { }
    ...
    void add_edge(vertex_id_type source, vertex_id_type target,
      ...
      const procid_t owning_proc = 
        base_type::edge_decision.edge_to_proc_greedy_skew(source, target, dht[source], dht[target], proc_num_edges, base_type::graph.skew_vector,usehash, userecent);
      obliv_lock.unlock();
